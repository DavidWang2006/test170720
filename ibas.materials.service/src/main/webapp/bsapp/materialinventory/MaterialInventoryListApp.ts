...
export class MaterialInventoryListApp extends ibas.BOListApplication<IMaterialInventoryListView, bo.MaterialInventory> {
    ...
    /** 应用标识 */
    static APPLICATION_ID: string = "20d66780-c556-474d-bd60-9961e663d4c4";
    /** 应用名称 */
    ...
    /** 删除数据，参数：目标数据集合 */
    protected deleteData(data: bo.MaterialInventory): void {
        // 检查目标数据
        if (ibas.objects.isNull(data)) {
            this.messages(ibas.emMessageType.WARNING, ibas.i18n.prop("sys_shell_please_chooose_data",
        ...
        if (data instanceof Array ) {
            for (let item of data) {
                if (ibas.objects.instanceOf(item, bo.MaterialInventory)) {
                    item.delete();
                    beDeleteds.add(item);
                }
            }
